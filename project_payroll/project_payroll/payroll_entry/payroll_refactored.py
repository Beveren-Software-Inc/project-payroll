...
class PayrollEntryOverride(PayrollEntry):
    ...
    def debug_salary_slips(self):
        ...
        {', '.join([f"{ss.employee} ({ss.name})" for ss in draft_salary_slips])}
        """)
    ...
    def make_accrual_jv_entry(self, submitted_salary_slips=None):
        ...
        if earnings or deductions:
            ...
            if employee_wise_accounting_enabled:
                ...
                # Standard accounting: Calculate payable from earnings and deductions
                ...
                # Get employee-wise payable amounts from salary slips for project allocation
                net_payables_by_employee = self.get_net_payable_per_employee()
                total_net_pay = sum(net_payables_by_employee.values())
                
                for employee_id, net_pay in net_payables_by_employee.items():
                    if flt(net_pay) == 0:
                        continue

                    # Calculate employee's share of total payable based on their net pay proportion
                    if total_net_pay > 0:
                        employee_payable_share = (net_pay / total_net_pay) * total_payable
                    else:
                        employee_payable_share = 0
                    ...
                    if employee_projects:
                        ...
                        for project_info in employee_projects:
                            project_payable_amount = employee_payable_share * (project_info["percent_pay"] / 100)
                        ...
                        # No project allocation - use default cost center
                        (exchange_rate, payable_amt) = self.get_amount_and_exchange_rate_for_journal_entry(
                            payroll_payable_account, employee_payable_share, company_currency, currencies
                        )
